...
/** 
 * Exports Pubmed format
 * 
 * @author Augustin Luna <augustin@mail.nih.gov>
 */
public class PubmedExporterHelper {
	 ...
	 *            the pathway
	 */
	PubmedExporterHelper(Pathway pw) {
		this.pw = pw;
	 ...
	 * @return the string
	 */
	private String generatePubmed(PathwayElement pwElem) {
		...
		for (org.pathvisio.biopax.reflect.BiopaxElement bpElem : bpElemColl) {
			...
			pubmedInst = "PMID- " + gpmlPubXRef.getPubmedId() + "\n";

			for (String author : gpmlPubXRef.getAuthors()) {
				pubmedInst += "AU  - " + author + "\n";	
			}
			
			pubmedInst += "SO  - " + gpmlPubXRef.getSource() + "\n";	
			pubmedInst += "DP  - " + gpmlPubXRef.getYear() + "\n";	
			pubmedInst += "TI  - " + gpmlPubXRef.getTitle() + "\n";	

			pubmedList.add(pubmedInst);

			Logger.log.debug("Pubmed Instance: " + pubmedInst);
		}

		Set<String> set = new HashSet<String>(pubmedList);
		List<String> uniqPubmedList = new ArrayList<String>(set);
		
		String pubmed = join(uniqPubmedList, "\n", false);

		Logger.log.debug("Pubmed: " + pubmed);
