...
import gov.nih.nci.lmp.refExport.BibtexExporterHelper;
...
/** 
 * Sets up the BibTeX format in Pathvisio
 * 
 * @author Augustin Luna <augustin@mail.nih.gov>
 */
public class BibtexFormat implements PathwayExporter {
	/** The extensions to add to the Pathvisio interface. */
	private final String[] EXTENSIONS = new String[] { "bib" };
	...
	public void doExport(File file, Pathway pathway) throws ConverterException {
		try {
			BibtexExporterHelper helper = new BibtexExporterHelper(pathway);
			helper.export(file);
		} catch (IOException e) {
